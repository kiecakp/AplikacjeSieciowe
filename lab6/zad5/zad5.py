...
ESMTP. Do wiadomości dodaj załącznik - dowolny obrazek (sprawdź format 
MIME: Multipart i Content-Type). Możesz wykorzystać openssl do przekonwertowania 
obrazka: cat obrazek |openssl base64.
"""
...
TEMAT = 'Zadanie 5 - Email z zalacznikiem (obrazek)'
WIADOMOSC = 'To jest testowa wiadomość e-mail wysłana za pomocą protokołu ESMTP.'

def load_base64_from_file(filename: str) -> str:
    with open(filename, 'r') as f:
        return f.read().strip()

boundary = 'BOUNDARY123'
encoded_image = load_base64_from_file('plik.txt')
