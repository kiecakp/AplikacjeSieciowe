...
o tym, ile bajtów (w sumie) zajmują wiadomości znajdujące się w skrzynce.
"""
...
class POP3Client:
    ...
    def stat(self):
        """
        Wysyła komendę STAT.
        Zwraca krotkę (liczba_wiadomości, łączny_rozmiar_w_bajtach).
        """
...
def main():
    ...
    try:
        ...
        # Pobranie liczby wiadomości i łącznego rozmiaru
        num_messages, total_size = client.stat()

        print()
        print("=" * 45)
        print(f"  Liczba wiadomości w skrzynce : {num_messages}")
        print(f"  Łączny rozmiar               : {total_size} B")
        print(f"  Łączny rozmiar               : {total_size / 1024:.2f} KB")
        print(f"  Łączny rozmiar               : {total_size / (1024 ** 2):.4f} MB")
        print("=" * 45)
